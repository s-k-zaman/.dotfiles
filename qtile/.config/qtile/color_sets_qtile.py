# If using transparency, make sure you add (background="#00000000") to 'Screen' line(s).
# Then, you can use RGBA color codes to add transparency to the colors below.
# For ex: colors = [["#282c34ee", "#282c34dd"], ...

from os import walk


DoomOne = [
    ["#282c34", "#282c34"],  # bg
    ["#bbc2cf", "#bbc2cf"],  # fg
    ["#1c1f24", "#1c1f24"],  # color01
    ["#ff6c6b", "#ff6c6b"],  # color02
    ["#98be65", "#98be65"],  # color03
    ["#da8548", "#da8548"],  # color04
    ["#51afef", "#51afef"],  # color05
    ["#c678dd", "#c678dd"],  # color06
    ["#46d9ff", "#46d9ff"],  # color15
]

Dracula = [
    ["#282a36", "#282a36"],  # bg
    ["#f8f8f2", "#f8f8f2"],  # fg
    ["#000000", "#000000"],  # color01
    ["#ff5555", "#ff5555"],  # color02
    ["#50fa7b", "#50fa7b"],  # color03
    ["#f1fa8c", "#f1fa8c"],  # color04
    ["#bd93f9", "#bd93f9"],  # color05
    ["#ff79c6", "#ff79c6"],  # color06
    ["#9aedfe", "#9aedfe"],  # color15
]

GruvboxDark = [
    ["#282828", "#282828"],  # bg
    ["#ebdbb2", "#ebdbb2"],  # fg
    ["#000000", "#000000"],  # color01
    ["#fb4934", "#fb4934"],  # color02
    ["#98971a", "#98971a"],  # color03
    ["#d79921", "#d79921"],  # color04
    ["#83a598", "#83a598"],  # color05
    ["#d3869b", "#d3869b"],  # color06
    ["#b8bb26", "#b8bb26"],  # color11
]
MonokaiPro = [
    ["#2D2A2E", "#2D2A2E"],  # bg
    ["#FCFCFA", "#FCFCFA"],  # fg
    ["#403E41", "#403E41"],  # color01
    ["#FF6188", "#FF6188"],  # color02
    ["#A9DC76", "#A9DC76"],  # color03
    ["#FFD866", "#FFD866"],  # color04
    ["#FC9867", "#FC9867"],  # color05
    ["#AB9DF2", "#AB9DF2"],  # color06
    ["#78DCE8", "#78DCE8"],  # color07
]

Nord = [
    ["#2E3440", "#2E3440"],  # bg
    ["#D8DEE9", "#D8DEE9"],  # fg
    ["#3B4252", "#3B4252"],  # color01
    ["#BF616A", "#BF616A"],  # color02
    ["#A3BE8C", "#A3BE8C"],  # color03
    ["#EBCB8B", "#EBCB8B"],  # color04
    ["#81A1C1", "#81A1C1"],  # color05
    ["#B48EAD", "#B48EAD"],  # color06
    ["#88C0D0", "#88C0D0"],  # color07
]

OceanicNext = [
    ["#1b2b34", "#1b2b34"],  # bg
    ["#d8dee9", "#d8dee9"],  # fg
    ["#29414f", "#29414f"],  # color01
    ["#ec5f67", "#ec5f67"],  # color02
    ["#99c794", "#99c794"],  # color03
    ["#fac863", "#fac863"],  # color04
    ["#6699cc", "#6699cc"],  # color05
    ["#c594c5", "#c594c5"],  # color06
    ["#5fb3b3", "#5fb3b3"],  # color07
]

Palenight = [
    ["#292d3e", "#292d3e"],  # bg
    ["#d0d0d0", "#d0d0d0"],  # fg
    ["#434758", "#434758"],  # color01
    ["#f07178", "#f07178"],  # color02
    ["#c3e88d", "#c3e88d"],  # color03
    ["#ffcb6b", "#ffcb6b"],  # color04
    ["#82aaff", "#82aaff"],  # color05
    ["#c792ea", "#c792ea"],  # color06
    ["#89ddff", "#89ddff"],  # color15
]

# TODO: make other colors in this format, include VIBGEOR color.
default_dark_transparent = dict(
    bar=["#000000b3", "#000000b3"],
    # bar=["#00000047", "#00000047"],
    border_normal=["#1b4965", "#1b4965"],
    border_focus=["#81A1C1", "#81A1C1"],
    shade1=["#434758", "#2c2c34"],  # solid
    shade2=["#2c2c346e", "#2c2c346e"],  # subtle transparent
    shade3=["#b1b7d1bf", "#b1b7d1bf"],  # inverted/stand out
    dark1=["#2c2c34", "#2c2c34"],  # 1-> darker
    dark2=["#656C79", "#656C79"],  # 2-> lighter
    light1=["#edf2f4", "#edf2f4"],
    light2=["#f8edeb", "#f8edeb"],
    danger=["#FF6188", "#FF6188"],
    highlight1=["#ffcb6b", "#ffcb6b"],
    highlight2=["#b58900", "#b58900"],
    highlight3=["#52b788", "#52b788"],
    highlight4=["#40916c", "#40916c"],
    highlight5=["#468faf", "#468faf"],
    highlight6=["#2c7da0", "#2c7da0"],
    highlight7=["#9d4edd", "#9d4edd"],
    highlight8=["#7b2cbf", "#7b2cbf"],
    highlight9=["#ff6000", "#ff6000"],
    highlight10=["#bc3908", "#bc3908"],
)
default_dark = dict(
    bg=["#292d3e", "#292d3e"],
    fg=["#d0d0d0", "#d0d0d0"],
    white=["#ffffff", "#ffffff"],
    shade1=["#434758", "#434758"],
    red=["#f07178", "#f07178"],
    green=["#c3e88d", "#c3e88d"],
    yellow=["#ffcb6b", "#ffcb6b"],
    blue=["#82aaff", "#82aaff"],
    magenta=["#c792ea", "#c792ea"],
    cyan=["#89ddff", "#89ddff"],
)

SolarizedDark = [
    ["#002b36", "#002b36"],  # bg
    ["#839496", "#839496"],  # fg
    ["#073642", "#073642"],  # color01
    ["#dc322f", "#dc322f"],  # color02
    ["#859900", "#859900"],  # color03
    ["#b58900", "#b58900"],  # color04
    ["#268bd2", "#268bd2"],  # color05
    ["#d33682", "#d33682"],  # color06
    ["#2aa198", "#2aa198"],  # color15
]

SolarizedLight = [
    ["#fdf6e3", "#fdf6e3"],  # bg
    ["#657b83", "#657b83"],  # fg
    ["#ece5ac", "#ece5ac"],  # color01
    ["#dc322f", "#dc322f"],  # color02
    ["#859900", "#859900"],  # color03
    ["#b58900", "#b58900"],  # color04
    ["#268bd2", "#268bd2"],  # color05
    ["#d33682", "#d33682"],  # color06
    ["#2aa198", "#2aa198"],  # color15
]

TomorrowNight = [
    ["#1d1f21", "#1d1f21"],  # bg
    ["#c5c8c6", "#c5c8c6"],  # fg
    ["#373b41", "#373b41"],  # color01
    ["#cc6666", "#cc6666"],  # color02
    ["#b5bd68", "#b5bd68"],  # color03
    ["#e6c547", "#e6c547"],  # color04
    ["#81a2be", "#81a2be"],  # color05
    ["#b294bb", "#b294bb"],  # color06
    ["#70c0ba", "#70c0ba"],  # color15
]

# theme/color to use
colors = default_dark_transparent
# adding some default colors
colors["black"] = ["#000000", "#000000"]
colors["violet"] = ["#EE82EE", "#EE82EE"]
colors["indigo"] = ["#4B0082", "#4B0082"]
colors["blue"] = ["#0000FF", "#0000FF"]
colors["green"] = ["#00FF00", "#00FF00"]
colors["yellow"] = ["#FFFF00", "#FFFF00"]
colors["orange"] = ["#FFA500", "#FFA500"]
colors["red"] = ["#FF0000", "#FF0000"]
colors["white"] = ["#ffffff", "#ffffff"]
colors["transparent"] = ["#00000000", "#00000000"]
