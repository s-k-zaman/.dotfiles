local utils = require("modules.venv_selector_python.utils")
local config = require("plugins.configs.venv_selector_python")
...
M.find_venvs = function(directories, max_depth, names)
	...
	directories = custom_utils.remove_duplicate_from_table(directories)
	...
	for _, venv in ipairs(venv_list) do
		if
			(custom_utils.clean_regex_from_string(venv.path)):find(
				custom_utils.clean_regex_from_string(vim.fn.getcwd())
			)
		then
...
function M.get_names_and_paths(lines)
	...
	for row in lines do
		if row ~= "" then
			table.insert(venv_lines, utils.remove_last_slash(row))
		end
	end
	-- Important: Remove duplicate paths
	venv_lines = custom_utils.remove_duplicate_from_table(venv_lines)
	for _, row in ipairs(venv_lines) do
...
M.set_venv = function(venv, run_post_set_function)
	...
	-- allow same vevn selection / if same venv do nothing
